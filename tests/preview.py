...
GALLERY_MD = _ROOT / "docs" / "gallery.md"
...
#: gallery.md の前書き。preview.html と違い**GitHub上でそのまま読める**ことが
#: 目的で、利用者の大半はリポジトリをcloneしないという前提に立っている
_GALLERY_HEAD = """<!-- Generated by tests/preview.py --update. Do not edit by hand. -->
...
def _pairs(names: list[str]) -> list[list[str]]:
    """light/dark の対を1行にまとめる。

    縦に積むとページが倍の長さになり、対の見比べもしづらい。GitHub の
    Markdown は連続する <img> を同じ行に並べるので、対を隣に置ける。
    対になっていないもの（片方しか定義が無い）は単独で1行にする。
    """
    out: list[list[str]] = []
    taken: set[str] = set()
    for name in names:
        ...
        if name.endswith("-light"):
            dark = name[: -len("-light")] + "-dark"
            if dark in names:
                out.append([name, dark])
                taken.update((name, dark))
                continue
        out.append([name])
        taken.add(name)
    return out
...
def build_gallery() -> str:
    """docs/gallery.md を VARIANTS から組み立てる。

    preview.html と同じ定義を読むので、片方だけ古くなることがない。
    """
    parts = [_GALLERY_HEAD]
    for title, names in PREVIEW_SECTIONS:
        parts.append(f"\n## {SECTION_TITLES_EN.get(title, title)}\n")
        for row in _pairs(list(names)):
            label = " / ".join(f"`{n}`" for n in row)
            kwargs = VARIANTS[row[0]][0]
            settings = ", ".join(f"`{k}={v!r}`" for k, v in kwargs.items()) or "*(defaults)*"
            parts.append(f"{label} — {settings}\n")
            imgs = "".join(
                f'<img src="../tests/golden/{n}.svg" width="312" alt="{n}"> ' for n in row)
            parts.append(imgs.rstrip() + "\n")
    parts.append(
...
def update() -> list[str]:
    ...
    if not GALLERY_MD.exists() or GALLERY_MD.read_text(encoding="utf-8") != gallery:
        GALLERY_MD.write_text(gallery, encoding="utf-8")
        changed.append("docs/gallery.md")
    return changed
